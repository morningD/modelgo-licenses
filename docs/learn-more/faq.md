...
MG licenses only apply to the Model, but their terms govern the <mark style="color:purple;">use and distribution of the Model and its derivatives, as well as outputs thereof, and complementary code and scripts</mark>. We classify these objects into four categories to define the scope of terms: Licensed Materials, Derivative Materials, Extracted Models, and Output, each with different restriction terms and usage policies. Extracted Models are models created by transferring patterns from the Model's output, such as through distillation methods. For example, according to the terms of MG-BY-ND-2.0, sharing the verbatim copy of Licensed Materials is allowed while sharing any Derivative Materials or Extracted Models is prohibited.
...
| License Name | Revocable? | Sublicensable? |
|---|---|---|
