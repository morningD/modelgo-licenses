---
title: 常见问题 - ModelGo 许可证 FAQ
description: 关于 ModelGo 许可证的常见问题解答，包括与 OpenRAIL、AI2 ImpACT 的区别，以及许可证兼容性和混音规则。
---
...
MG 许可证仅适用于模型，但其条款管辖 <mark style="color:purple;">模型及其衍生品的使用和分发，以及其输出和配套代码和脚本</mark>。我们将这些对象分为四类来定义条款范围：许可材料（Licensed Materials）、衍生材料（Derivative Materials）、提取模型（Extracted Models）和输出（Output），每类有不同的限制条款和使用政策。提取模型是通过从模型输出中转移模式创建的模型，例如通过蒸馏方法。例如，根据 MG-BY-ND-2.0 的条款，允许共享许可材料的逐字副本，但禁止共享任何衍生材料或提取模型。
...
<mark style="color:purple;">**目前不是。**</mark> ModelGo 许可证的任何变体都尚未获得开源促进会 (OSI) 的批准。但是，我们的目标是让 MG-BY-SA-2.0 保持模型"开放"。值得一提的是，"开放"的含义在软件和模型之间可能有所不同。在 ModelGo 许可证中，SA 意味着**许可材料**、**衍生材料**和**提取模型**的可用性，不包括数据集的开放性（因为我们认为数据集不在 MG 许可证的管辖范围内）。

## 我可以对使用带有 SA 的 MG 许可的模型闭源吗？

<mark style="color:purple;">**不可以。**</mark> 这也适用于其衍生品及提取模型。MG-BY-SA-2.0 是一个 Copyleft 许可证，意味着所有衍生作品及提取模型也必须受相同许可证的保护。您也不能撤销 MG-BY-SA-2.0，这不同于 GPL-3.0，后者没有明确声明授予的专利许可是不可撤销的。
