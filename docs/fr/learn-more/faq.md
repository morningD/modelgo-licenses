...
Les licences MG s'appliquent uniquement au Modèle, mais leurs termes régissent l'<mark style="color:purple;">utilisation et la distribution du Modèle et de ses dérivés, ainsi que des sorties correspondantes, et du code et des scripts complémentaires</mark>. Nous classons ces objets en trois catégories pour définir la portée des termes : Licensed Materials, Derivative Materials et Output, chacune avec des conditions de restriction et des politiques d'utilisation différentes. Par exemple, selon les termes de MG-BY-ND-2.0, le partage de la copie conforme des Licensed Materials est autorisé tandis que le partage de tout Derivative Materials est interdit.
